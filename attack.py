...
def attack(model: Callable,
    ...
    for s_m in s_models:
        for attack in attacks:
            # adv_img = attack.attack(s_m, t_image, t_label, targeted, diff_flag=False).squeeze(0)
            ...
            if norm > 0:
                # Run bound search
                lower, upper, found, num1 = bound_search(model, image,
                                                   label, delta,
                                                   targeted=targeted)
                if found:
                ...
                # Run binary search
                upper_, found_, num2 = binary_search(model, image, label,
            ...
            count = count + 1
            test_num = num1 + num2 + test_num
    return adversarial, count, test_num


def bound_search(model, image, label, delta, alpha=1, iters=19, targeted=False):
...
def binary_search(model, image, label, lower, upper, steps=10, targeted=False,test_num=0):

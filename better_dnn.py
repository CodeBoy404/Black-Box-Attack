...
class DDN:
    ...
    def attack(self, model: nn.Module, inputs: torch.Tensor, labels: torch.Tensor,
               targeted: bool = False, diff_flag: bool = False) -> torch.Tensor:
        """
        ...
        for i in range(self.steps):
            ...
            l2 = delta.data.contiguous().view(batch_size, -1).norm(p=2, dim=1)
            adv = inputs + delta
            logits = model(adv)
            ...
            delta.data.mul_((norm / delta.data.contiguous().view(batch_size, -1).norm(2, 1)).view(-1, 1, 1, 1))
